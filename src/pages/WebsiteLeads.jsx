...
export default function WebsiteLeads() {
    ...
    const [activeTab, setActiveTab] = useState('Activity History'); // 'Activity History' | 'Task'
    const [isReportColumnOpen, setIsReportColumnOpen] = useState(true);

    // Mock Data
    const leads = [
        { id: 1, name: 'Abdul Rehman Warraich', phone: '923130736125', status: 'Fresh', rating: 1, time: '5m' },
        { id: 2, name: 'Roman Khan', phone: '923409444190', status: 'Fresh', rating: 0, time: '12m' },
        { id: 3, name: 'Muhammad Umer', phone: '923246871850', status: 'Fresh', rating: 0, time: '1h' },
        { id: 4, name: 'Danial Ahmad', phone: '923354830077', status: 'Re-Engaged', rating: 1, time: '2h' },
        { id: 5, name: 'Sardar Hanzalah Ali', phone: '923062436210', status: 'Not Sure', rating: 0, time: '3h' },
        { id: 6, name: 'Shahbaz Ali', phone: '93789268046', status: 'Fresh', rating: 0, time: '4h' },
        { id: 7, name: 'Hammad Saleem', phone: '923049995349', status: 'Fresh', rating: 0, time: '5h' },
    ];
        ...
        <div className="flex h-screen bg-[#F8F9FA] text-[#202124] font-sans">
            ...
            <div className="flex flex-1 flex-col overflow-hidden">
                ...
                <main className="flex-1 overflow-hidden flex flex-col lg:flex-row relative">
                    ...
                    {/* Expand Button (Visible only when column is closed) */}
                    {!isReportColumnOpen && (
                        <button
                            ...
                            <div className="flex items-center justify-between bg-gradient-to-br from-teal-50/50 to-white border border-teal-50 p-3 rounded-xl shadow-sm">
                                ...
                                <div className="relative w-11 h-11 flex items-center justify-center">
                                    <svg className="w-full h-full transform -rotate-90">
                                        ...
                                        <circle cx="22" cy="22" r="17" stroke="currentColor" strokeWidth="4" fill="transparent" className="text-[#08A698]" strokeDasharray="100" strokeDashoffset="38" strokeLinecap="round" />
                                    </svg>
                                    <span className="absolute text-[10px] font-bold text-gray-700">62%</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="divide-y divide-gray-50">
                            <AccordionItem title="Campaign Assignees Report" isOpen={true}>
                                <div className="flex items-center justify-center py-4">
                                    <div className="w-24 h-24 rounded-full bg-[conic-gradient(at_center,_#08A698_0deg_180deg,_#E2E8F0_180deg_360deg)] relative shadow-md ring-4 ring-white">
                                        <div className="absolute inset-3 bg-white rounded-full flex items-center justify-center">
                                            <span className="text-xs font-bold text-gray-400">Total</span>
                                        </div>
                                    </div>
                                    <div className="ml-4 text-xs space-y-2">
                                        <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-[#08A698] shadow-sm"></span> <span className="font-medium">Aiman (50%)</span></div>
                                        <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-gray-300 shadow-sm"></span> <span className="text-gray-500">Others (50%)</span></div>
                                    </div>
                                </div>
                            </AccordionItem>
                            ...
                            <AccordionItem title="Calls Status Report" />
                        </div>
                    </div>
                    ...
                    <div className={`w-full bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ${isReportColumnOpen ? 'lg:w-[30%]' : 'lg:w-[35%]'}`}>
                        ...
                        <div className="flex border-b border-gray-200 bg-white">
                            ...
                            <button className="flex-1 py-3 text-sm font-bold text-[#08A698] border-b-2 border-[#08A698] bg-teal-50/10">NEW (4919)</button>
                        </div>
                    ...
                    <div className="flex-1 bg-[#F8F9FA] flex flex-col overflow-hidden relative">
                        ...
                        <div className="bg-white/80 backdrop-blur-md p-6 border-b border-gray-200 shadow-sm z-20 sticky top-0">
                            ...
                            {/* Info Grid with Hover Edit Effect */}
                            <div className="grid grid-cols-2 gap-y-5 gap-x-12 mb-6">
                                <div className="space-y-5">
                                    <InfoField label="Do you want to earn Millions" value="hell...yes" icon={HashtagIcon} editable />
                                    <InfoField label="Alternate Phone" value="923001234567" icon={PhoneIcon} flag editable />
                                    <InfoField label="Facebook Campaign" value="1 Candle Course" icon={MegaphoneIcon} editable />
                                    <InfoField label="City" value="Empty" icon={MapPinIcon} isPlaceholder editable />
                                </div>
                                <div className="space-y-5">
                                    <InfoField label="Phone" value={selectedLead.phone} icon={PhoneIcon} flag editable />
                                    <InfoField label="Facebook Ad" value="1 Candle Course" icon={MegaphoneIcon} editable />
                                    <InfoField label="Lead ID Facebook" value="25561892994" icon={HashtagIcon} editable />
                                    <InfoField label="Age" value="Empty" icon={UserIcon} isPlaceholder editable />
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-gray-50/50">
                            ...
                            <div className="relative pl-2 space-y-0">
                                ...
                                <TimelineItem
                                    ...
                                    text="Lead Capture from Form: Game of Forex Form 2 & Page: Game of Forex By Waqas"
                                    time="12m"
                                    isLast={false}
                                />
                                <TimelineItem
                                ...
                                />
                                <TimelineItem
                                    icon={<HashtagIcon className="w-3.5 h-3.5 text-gray-600" />}
                                    bg="bg-gray-100 border-gray-200"
                                    text="ad_id: 120236222114250570"
                                    time="26m"
                                    isLast={true}
                                />
                            </div>
...
const StatusBadge = ({ status }) => {
    const styles = {
        ...
        'Re-Engaged Lead': 'bg-purple-50 text-purple-700 border-purple-200 ring-1 ring-purple-100 shadow-sm'
    };
    const defaultStyle = 'bg-gray-50 text-gray-600 border-gray-200';

    let badgeStyle = defaultStyle;
    if (status.includes('Fresh')) badgeStyle = styles['Fresh'];
    else if (status.includes('Not Sure')) badgeStyle = styles['Not Sure'];
    else if (status.includes('Re-Engaged')) badgeStyle = styles['Re-Engaged'];

    return (
        <span className={`px-2.5 py-0.5 rounded-md text-[10px] font-bold border ${badgeStyle} uppercase tracking-wider`}>
            {status}
        </span>
    );
};
